...
// ฟังก์ชันดึงกระทู้กีฬาทั้งหมด
function getSportPosts($pdo, $subcategory = null) {
...
// ฟังก์ชันดึงประเภทย่อยทั้งหมดของหมวดกีฬา
function getSportSubcategories($pdo) {
    $sql = "SELECT DISTINCT subcategory FROM posts WHERE category = 'sports' AND subcategory IS NOT NULL";
    $stmt = $pdo->query($sql);
    return $stmt->fetchAll(PDO::FETCH_ASSOC);
}

// ดึงประเภทย่อยทั้งหมด
$subcategories = getSportSubcategories($pdo);

// ตรวจสอบว่ามีการเลือกประเภทย่อยหรือไม่
$selected_subcategory = isset($_GET['subcategory']) ? $_GET['subcategory'] : null;

// ดึงกระทู้กีฬา
$sport_posts = getSportPosts($pdo, $selected_subcategory);
...
// ฟังก์ชันดึงกีฬายอดนิยม 5 อันดับ
function getPopularSports($pdo, $limit = 5) {
...
// ดึงกีฬายอดนิยม
$popular_sports = getPopularSports($pdo);
...
<head>
    ...
    <title>กระทู้กีฬา - ATJ HUB</title>
...
<body>
        ...
        <div class="container-fluid">
            ...
            <div class="collapse navbar-collapse" id="navbarNav">
                <ul class="navbar-nav me-auto mb-2 mb-lg-0">
                    ...
                    <li class="nav-item"><a class="nav-link active" href="sport.php">กีฬา</a></li>
    ...
    <div class="container mt-5">
        <h1 class="category-title">กระทู้กีฬา</h1>
        
        <!-- ส่วนแสดงกีฬายอดนิยม -->
        <h3 class="mb-3">กีฬามาแรง</h3>
        <div class="popular-news mb-4">
            <?php foreach ($popular_sports as $sport): ?>
                <div class="popular-news-item">
                    <div class="card h-100">
                        <?php if (!empty($sport['image_url'])): ?>
                            <img src="<?php echo htmlspecialchars($sport['image_url']); ?>" class="card-img-top popular-news-image" alt="รูปภาพกีฬา">
                        <?php endif; ?>
                        <div class="card-body d-flex flex-column">
                            <h6 class="card-title"><?php echo htmlspecialchars(substr($sport['title'], 0, 50)) . '...'; ?></h6>
                            <div class="mt-auto">
                                <div class="d-flex justify-content-between align-items-center">
                                    <small class="text-muted">
                                        <i class="bi bi-hand-thumbs-up"></i> <?php echo $sport['likes_count']; ?>
                                        <i class="bi bi-chat-dots ms-2"></i> <?php echo $sport['comments_count']; ?>
                                    </small>
                                </div>
                                <button class="btn btn-sm btn-primary mt-2 read-more" data-post-id="<?php echo $sport['id']; ?>" data-bs-toggle="modal" data-bs-target="#postModal">อ่านเพิ่มเติม</button>
                            </div>
                        </div>
                    </div>
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // ฟังก์ชันสำหรับอัปเดตจำนวนการถูกใจ
    ...
    // ฟังก์ชันสำหรับแสดงหรือซ่อนความคิดเห็น
    ...
    // ฟังก์ชันสำหรับแสดงเนื้อหากระทู้
    function displayPostContent(data) {
    ...
    // เรียกใช้ฟังก์ชันเพื่อเพิ่ม Event Listeners
    addEventListeners();
