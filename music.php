...
// ฟังก์ชันดึงกระทู้เพลงทั้งหมด
function getMusicPosts($pdo, $subcategory = null) {
...
// ฟังก์ชันดึงประเภทย่อยทั้งหมดของหมวดเพลง
function getMusicSubcategories($pdo) {
    $sql = "SELECT DISTINCT subcategory FROM posts WHERE category = 'music' AND subcategory IS NOT NULL";
    $stmt = $pdo->query($sql);
    return $stmt->fetchAll(PDO::FETCH_ASSOC);
}

// ดึงประเภทย่อยทั้งหมด
$subcategories = getMusicSubcategories($pdo);

// ตรวจสอบว่ามีการเลือกประเภทย่อยหรือไม่
$selected_subcategory = isset($_GET['subcategory']) ? $_GET['subcategory'] : null;

// ดึงกระทู้เพลง
$music_posts = getMusicPosts($pdo, $selected_subcategory);
...
// ฟังก์ชันดึงเพลงยอดนิยม 5 อันดับ
function getPopularMusic($pdo, $limit = 5) {
...
// ดึงเพลงยอดนิยม
$popular_music = getPopularMusic($pdo);
...
<head>
    ...
    <title>กระทู้เพลง - ATJ HUB</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0-alpha1/dist/css/bootstrap.min.css" rel="stylesheet">
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.7.2/font/bootstrap-icons.css">
    <style>
        /* ใช้ CSS เดียวกับ sport.php และ games.php */
        body {
...
<body>
        ...
        <div class="container-fluid">
            ...
            <div class="collapse navbar-collapse" id="navbarNav">
                <ul class="navbar-nav me-auto mb-2 mb-lg-0">
                    ...
                    <li class="nav-item"><a class="nav-link active" href="music.php">เพลง</a></li>
    ...
    <div class="container mt-5">
        <h1 class="category-title">กระทู้เพลง</h1>
        
        <!-- ส่วนแสดงเพลงยอดนิยม -->
        <h3 class="mb-3">มาแรง</h3>
        <div class="popular-news mb-4">
            <?php foreach ($popular_music as $music): ?>
                <div class="popular-news-item">
                    <div class="card h-100">
                        <?php if (!empty($music['image_url'])): ?>
                            <img src="<?php echo htmlspecialchars($music['image_url']); ?>" class="card-img-top popular-news-image" alt="รูปภาพเพลง">
                        <?php endif; ?>
                        <div class="card-body d-flex flex-column">
                            <h6 class="card-title"><?php echo htmlspecialchars(substr($music['title'], 0, 50)) . '...'; ?></h6>
                            <div class="mt-auto">
                                <div class="d-flex justify-content-between align-items-center">
                                    <small class="text-muted">
                                        <i class="bi bi-hand-thumbs-up"></i> <?php echo $music['likes_count']; ?>
                                        <i class="bi bi-chat-dots ms-2"></i> <?php echo $music['comments_count']; ?>
                                    </small>
                                </div>
                                <button class="btn btn-sm btn-primary mt-2 read-more" data-post-id="<?php echo $music['id']; ?>" data-bs-toggle="modal" data-bs-target="#postModal">อ่านเพิ่มเติม</button>
                            </div>
                        </div>
                    </div>
                </div>
