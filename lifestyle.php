...
// ฟังก์ชันดึงกระทู้ไลฟ์สไตล์ทั้งหมด
function getLifestylePosts($pdo, $subcategory = null) {
...
// ฟังก์ชันดึงประเภทย่อยทั้งหมดของหมวดไลฟ์สไตล์
function getLifestyleSubcategories($pdo) {
    $sql = "SELECT DISTINCT subcategory FROM posts WHERE category = 'lifestyle' AND subcategory IS NOT NULL";
    $stmt = $pdo->query($sql);
    return $stmt->fetchAll(PDO::FETCH_ASSOC);
}

// ดึงประเภทย่อยทั้งหมด
$subcategories = getLifestyleSubcategories($pdo);

// ตรวจสอบว่ามีการเลือกประเภทย่อยหรือไม่
$selected_subcategory = isset($_GET['subcategory']) ? $_GET['subcategory'] : null;

// ดึงกระทู้ไลฟ์สไตล์
$lifestyle_posts = getLifestylePosts($pdo, $selected_subcategory);
...
// ฟังก์ชันดึงไลฟ์สไตล์ยอดนิยม 5 อันดับ
function getPopularLifestyle($pdo, $limit = 5) {
...
// ดึงไลฟ์สไตล์ยอดนิยม
$popular_lifestyle = getPopularLifestyle($pdo);

// เพิ่มฟังก์ชันสำหรับดึงความคิดเห็น
...
<head>
    ...
    <title>กระทู้ไลฟ์สไตล์ - ATJ HUB</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0-alpha1/dist/css/bootstrap.min.css" rel="stylesheet">
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.7.2/font/bootstrap-icons.css">
    <style>
        /* ใช้ CSS เดียวกับ sport.php, games.php และ music.php */
        body {
...
<body>
        ...
        <div class="container-fluid">
            ...
            <div class="collapse navbar-collapse" id="navbarNav">
                <ul class="navbar-nav me-auto mb-2 mb-lg-0">
                    ...
                    <li class="nav-item"><a class="nav-link active" href="lifestyle.php">ไลฟ์สไตล์</a></li>
                </ul>
                <div class="d-flex align-items-center">
                    ...
                    <a class="dropdown-toggle d-flex align-items-center hidden-arrow" href="#" id="navbarDropdownMenuAvatar" role="button" data-bs-toggle="dropdown" aria-expanded="false">
                        ...
                        <span>
                        <?php 
                            if (isset($user) && is_array($user) && isset($user['username'])) {
    ...
    <div class="container mt-5">
        <h1 class="category-title">กระทู้ไลฟ์สไตล์</h1>
        
        <!-- ส่วนแสดงไลฟ์สไตล์ยอดนิยม -->
        <h3 class="mb-3">ไลฟ์สไตล์มาแรง</h3>
        <div class="popular-news mb-4">
            <?php foreach ($popular_lifestyle as $lifestyle): ?>
                <div class="popular-news-item">
                    <div class="card h-100">
                        <?php if (!empty($lifestyle['image_url'])): ?>
                            <img src="<?php echo htmlspecialchars($lifestyle['image_url']); ?>" class="card-img-top popular-news-image" alt="รูปภาพไลฟ์สไตล์">
                        <?php endif; ?>
                        <div class="card-body d-flex flex-column">
                            <h6 class="card-title"><?php echo htmlspecialchars(substr($lifestyle['title'], 0, 50)) . '...'; ?></h6>
                            <div class="mt-auto">
                                <div class="d-flex justify-content-between align-items-center">
                                    <small class="text-muted">
                                        <i class="bi bi-hand-thumbs-up"></i> <?php echo $lifestyle['likes_count']; ?>
                                        <i class="bi bi-chat-dots ms-2"></i> <?php echo $lifestyle['comments_count']; ?>
                                    </small>
                                </div>
                                <button class="btn btn-sm btn-primary mt-2 read-more" data-post-id="<?php echo $lifestyle['id']; ?>" data-bs-toggle="modal" data-bs-target="#postModal">อ่านเพิ่มเติม</button>
                            </div>
                        </div>
                    </div>
                </div>
